...
public class PersianCalendar extends Calendar {
     ...
     * that is set for the calendar.
     * The time is not recomputed first; to recompute the time, then the fields,
    ...
    public void add(int field, int amount) {
        ...
        if (field == MONTH) {
            ...
            add(YEAR, amount / 12);
            set(MONTH, amount % 12);
            if (get(DAY_OF_MONTH) > j_days_in_month[amount % 12]) {
                set(DAY_OF_MONTH, j_days_in_month[amount % 12]);
    ...
    public static PersianCalendar newInstance() {
        PersianCalendar instance = new PersianCalendar();
